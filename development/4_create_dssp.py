...
def apply_dssp(structure_file):
    os.system(f"mkdssp -i {structure_file} -o {structure_file.split('.')[0]}.dssp")

def main():
    uncorrected_dssp_files = []
    launch_mode = input("Choose launch mode for DSSP: 'locally' or 'remote'\n")
    ...
    for structure in structure_list:
        ...
        for structure_file in structure_files:
            ...
            if launch_mode == "locally":
                apply_dssp(structure_file)
                if not os.path.exists(f"{structure_file.split('.')[0]}.dssp"):
    ...
    if len(uncorrected_dssp_files) > 0:
        with open(os.path.join(StructureSet_path, f"{name_of_set}_UncorrectedDSSPFiles.txt"), 'w') as file:
