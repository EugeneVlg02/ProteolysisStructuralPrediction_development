    ...
    def process_STR(parts_STR):
        ...
        if len(set(part_2)) > 1:
            ...
            if len(part_3) < 3:
                ...
                elif part_3[:3] in ["UUU", "UUS", "UUT", "UUB", "BUS", "BUT", "BUU", "UBS", "UBT", "UBU", "UBB"]:
...
def main():
    ...
    for structure in structure_list:        
        ...
        for dssp_file in dssp_files:
            ...
            input_CW_file = os.path.join(sequence_path, f"{structure_name}_PDB_DSSP.fa")
            output_CW_file = os.path.join(sequence_path, f"{structure_name}_PDB_DSSP_ClustalO.fa")
            ...
            if ("is_cut" in final_data.columns) and ('AF-' in structure_name):
                final_columns = ["structure", "chain", "AA", "num_AA", "is_cut", "MEROPS_CODE", "SUBSTRATE_name", "init_SS_type", "SS_type", "bp1", "bp2", "ACC", "AF_score", "len_loop", "is_terminus"]
            elif ("is_cut" in final_data.columns) and ('AF-' not in structure_name):
                final_columns = ["structure", "chain", "AA", "num_AA", "is_cut", "MEROPS_CODE", "SUBSTRATE_name", "init_SS_type", "SS_type", "bp1", "bp2", "ACC", "bfac", "len_loop", "is_terminus"]
            elif ("is_cut" not in final_data.columns) and ('AF-' in structure_name):
                final_columns = ["structure", "chain", "AA", "num_AA", "init_SS_type", "SS_type", "bp1", "bp2", "ACC", "AF_score", "len_loop", "is_terminus"]
            else:
            ...
            final_data.to_csv(os.path.join(feature_path, f"{structure_name}_final.csv"), index=False)
